...
(function(runner) {
    ...
    runner.addTest('menu-version', 'Menu Screen', 'Version is displayed',
        'Verifies menu shows a version number',
        async () => {
            runner.resetGame();
            await runner.wait(100);
            const version = runner.getElement('.version');
            if (!version || !version.textContent.match(/VERSION\s+\d+\.\d+/i)) {
                throw new Error('Version should show VERSION X.X format');
            }
        }
    );
